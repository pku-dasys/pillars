...
class AdresVLIWPE5InBlock(name: String) extends BlockTrait{
  setName(name)
  hierName.append(name)
  isConfigRegion = true
...
class TileCompleteBlock(name : String, x : Int, y : Int, numIn : Int, numOut : Int) extends BlockTrait {
  setName(name)
  hierName.append(name)
  addOutPorts((0 to numOut-1).map(i => "out_" + i.toString).toArray)
  addInPorts((0 to numIn-1).map(i => "input_" + i.toString).toArray)
  ...
  for (i <- 0 until x) {
    for (j <- 0 until y) {
      ...
      val peW = peMap(((i - 1) + x) % x + j * x)
      if(j == 0){
        connectArray.append(List(List(ioBlock.getName() + "/" ,"neighbour_out_"+i.toString), List(peCurrent.getName()+ "/", "input_IO")))
        connectArray.append(List(List(peCurrent.getName()+ "/", "out_0"), List(ioBlock.getName() + "/" ,"neighbour_input_"+i.toString)))
        connectArray.append(List(List(peCurrent.getName()+ "/", "rf_out"), List(globalRFBlock.getName()+ "/" ,"input_"+i.toString)))
        connectArray.append(List(List(globalRFBlock.getName()+ "/" ,"out_"+(i * 2).toString), List(peCurrent.getName()+ "/", "input_rf_mux0")))
        connectArray.append(List(List(globalRFBlock.getName()+ "/" ,"out_"+(i * 2 + 1).toString), List(peCurrent.getName()+ "/", "input_rf_muxOut")))
      }
      connectArray.append(List(List(peCurrent.getName() + "/", "out_0"), List(peS.getName()+ "/", "input_n")))
      connectArray.append(List(List(peCurrent.getName() + "/", "out_0"), List(peN.getName()+ "/", "input_s")))
      connectArray.append(List(List(peCurrent.getName() + "/", "out_0"), List(peE.getName()+ "/", "input_w")))
