...
class Z3ProverMap {
  ...
  def getOmtModel(ctx: Context, separatedPR: Boolean, scheduleControl: Boolean): Array[Optimize] = {
    ...
    // initialize F(*,*)
    for ((funcNodeName, funcNodeId) <- mrrgFuncName.zipWithIndex; (opNodeName, opNodeId) <- dfgOpNodeName.zipWithIndex) {
      val isLegal = if (mrrgFuncOpcodes(funcNodeId).indexOf(dfgOpNodeOpcode(opNodeId)) >= 0) {
    ...
    // initialize R(*,*)
    for ((routNodeName, routNodeId) <- mrrgRoutName.zipWithIndex; (valNodeName, valNodeId) <- dfgValNodeName.zipWithIndex) {
  ...
  def constrFunctionExclusivity(ctx: Context, varF: Map[(Int,Int), IntExpr]): Iterable[BoolExpr] = {
    val one = ctx.mkInt(1)
    varF.groupBy{case ((funcNodeId,_),_) => funcNodeId}.flatMap{case (_,vf) => {
  ...
  def constrOperationPlacement(ctx: Context, varF: Map[(Int,Int), IntExpr]): Iterable[BoolExpr] = {
    val one = ctx.mkInt(1)
    varF.groupBy{case ((_,opNodeId),_) => opNodeId}.map{case (_,vf) => {
      val varList = vf.map(_._2).toSeq
  ...
  def constrRoutingResource(ctx: Context, varR: Map[(Int,Int), IntExpr], varS: Map[(Int,Int,Int), IntExpr]): Iterable[BoolExpr] = {
    val vs = varS.groupBy{case ((x,y,_),_) => (x,y)}
    varR.flatMap{
      case ((routNodeId, valNodeId), r) => {
        vs((routNodeId, valNodeId)).map{case (_, s) =>
  ...
  def constrRoutingExclusivity(ctx: Context, varR: Map[(Int,Int), IntExpr]): Iterable[BoolExpr] = {
    val one = ctx.mkInt(1)
    varR.groupBy{case ((routNodeId,_),_) => routNodeId}.flatMap{case (_,vr) => {
  ...
  def constrMultiplexerExclusivity(ctx: Context, varR: Map[(Int,Int), IntExpr]): Iterable[BoolExpr] = {
    val one = ctx.mkInt(1)
    varR.flatMap{case ((routNodeId, valNodeId), r) => {
      val faninRoutNodeIds = mrrgRoutFaninId(routNodeId).filter(mrrgRoutFaninType(_) == MRRG_ROUT_NODE_TYPE)
  ...
  def constrFanoutRouting(ctx: Context, varS: Map[(Int,Int,Int), IntExpr], varF: Map[(Int,Int), IntExpr]): Iterable[BoolExpr] = {
      ...
      for ((opNodeId, i) <- dfgValNodeOut(valNodeId).zipWithIndex) {
        ...
        for ((fanoutId,j) <- mrrgRoutFanoutId(routNodeId).zipWithIndex) {
          mrrgRoutFanoutType(routNodeId)(j) match {
            case MRRG_ROUT_NODE_TYPE => varList.append(varS((fanoutId, valNodeId, opNodeId)))
            case MRRG_FUNC_NODE_TYPE => {
              if (mrrgFuncFaninId(fanoutId).size > operand) {
                if (dfgCommutativeSet.contains(opNodeId) || mrrgFuncFaninId(fanoutId)(operand) == routNodeId) {
        ...
        if (varList.size > 0) {
          val sum = ctx.mkAdd(varList: _*)
          exprList.append(ctx.mkLe(varS((routNodeId, valNodeId, opNodeId)), sum))
        }
      }
    }
    exprList
  ...
  def constrInitialFanout(ctx: Context, varS: Map[(Int,Int,Int), IntExpr], varF: Map[(Int,Int), IntExpr]): Iterable[BoolExpr] = {
    val exprList = ArrayBuffer[BoolExpr]()
    varF.foreach{case ((funcNodeId,opNodeId),f) => {
      if (dfgOpNodeOut(opNodeId) != -1) {
        ...
        for (funcNodeFanoutId <- mrrgFuncFanoutId(funcNodeId)) {
          for (valNodeFanoutId <- dfgValNodeOut(valNodeId)) {
            ...
            val expr = ctx.mkEq(f, varS((funcNodeFanoutId, valNodeId, valNodeFanoutId)))
