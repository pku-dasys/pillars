...
//            val VectorB = (100 until dataSize + 100).toArray
      val VectorA = (0 until dataSize)
        .map(_ => Math.abs(scala.util.Random.nextInt() % 1000))
        .toArray
      val VectorB = (0 until dataSize)
        .map(_ => Math.abs(scala.util.Random.nextInt() % 1000))
        .toArray
...
//            val inputIndexes = (0 until dataSize).toList.reverse
      val expectedRet = (0 until dataSize).map(i =>
        VectorA(inputIndexes(i)) + VectorB(inputIndexes(i))
      )
      ...
      val constOpNames =
        dfg.opNodes.filter(op => op.opcode == OpEnum.CONST).map(op => op.name)
      val constValue = (0 until constOpNames.size)
        .map(i => ConstValue(constOpNames(i), constVals(i)))
        .toList

      //Operator incr0 should generate (j <- 0 until dataSize).
      //So the parameter of the counter is (init = 0, step = 1, end = dataSize, freq = 1)
      val counterOpNames =
        dfg.opNodes.filter(op => op.opcode == OpEnum.INCR).map(op => op.name)
      val counterConfig = List(
        CounterConfig(counterOpNames(0), 0, 1, dataSize, 1)
      )
      ...
      val inputToSRAM = (0 until numSRAM)
        .map(i => InputToSRAM(i, a_base, VectorA.toList))
        .toList :::
        (0 until numSRAM)
          .map(i => InputToSRAM(i, b_base, VectorB.toList))
          .toList

      val outputFromSRAM = List(
        OutputFromSRAM(3, a_base, VectorA.reverse.toList)
      )

      //Please make sure there are 2 operators with INPUT opcode in the DFG.
      val inputOpNames =
        dfg.opNodes.filter(op => op.opcode == OpEnum.INPUT).map(op => op.name)
      ...
      val outputOpNames =
        dfg.opNodes.filter(op => op.opcode == OpEnum.OUTPUT).map(op => op.name)
      //      val outputFromPort = List(OutputFromPort(outputOpNames(0), outResult.toList))
      val outputFromPort = List(
        OutputFromPort(outputOpNames(0), expectedRet.toList)
      )
    ...
    solver match {
      case Solver.Gurobi =>
        ILPMap.mapping(
          dfg,
          MRRG,
          filename = mappingResultFilename,
          separatedPR = separatedPR,
      ...
      case Solver.Search =>
        SearchMap.mapping(
          dfg,
          MRRG,
          mappingResultFilename,
          scheduleControl = scheduleControl,
          skewLimit = 4
        )
      case Solver.Z3Prover =>
        OmtMap.mapping(
          dfg,
          MRRG,
          filename = mappingResultFilename,
...
/** A tester for synthesized design of matrix multiplication.
  ...
  */
class SynthesizedModuleTester(
    c: SynthesizedModule,
    input: Array[Int],
    outResult: Array[Int],
    expectedDataInSRAM: Array[Int],
    outputCycle: Int
  ...
  println(
    "Checking the results from the output port(s) of the synthesized module."
  )
  for (t <- 0 until T) {
    if (t < dataSize) {
    ...
    if (t >= outputCycle) {
      ...
      println(
        outResult(t - outputCycle).toString + " " + peek(c.io.outs(0))
          .toString()
      )
    }
    step(1)
  ...
  for (t <- 0 until expectedDataInSRAM.size) {
    ...
    println(
      expectedDataInSRAM(t).toString + " " + peek(c.io.storeUnitMemDatas(0))
    )
  }
}
